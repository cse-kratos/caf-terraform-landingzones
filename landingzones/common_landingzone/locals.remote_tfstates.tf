...
locals {
  ...
  combined = {
    vnets                            = merge(local.remote.vnets, map(var.landingzone.key, module.commonzone.vnets))
    azurerm_firewalls                = merge(local.remote.azurerm_firewalls, map(var.landingzone.key, module.commonzone.azurerm_firewalls))
    public_ip_addresses              = merge(local.remote.public_ip_addresses, map(var.landingzone.key, module.commonzone.public_ip_addresses))
    virtual_wans                     = merge(local.remote.virtual_wans, map(var.landingzone.key, module.commonzone.virtual_wans))
    private_dns                      = merge(local.remote.private_dns, map(var.landingzone.key, module.commonzone.private_dns))
    application_gateways             = merge(local.remote.application_gateways, map(var.landingzone.key, module.commonzone.application_gateways))
    application_gateway_applications = merge(local.remote.application_gateway_applications, map(var.landingzone.key, module.commonzone.application_gateway_applications))
  }
